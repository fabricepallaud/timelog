...
export default {
  ...
  methods: {
    ...
    addEntry: function (day, hours, dayInput) {
      ...
      const timeDay = db.collection('times')
      .where('date', '==', day)
      .where('projectId', '==', this.projectId)
      .where('taskId', '==', this.taskId)
      .where('userId', '==', this.$store.state.userId)
  ...
  mounted() {
    ...
    const project = db.collection('projects').doc(this.projectId)
    project.get().then((doc) => {
      if (doc.exists) {
        ...
        const client = db.collection('clients').doc(this.clientId)
        client.get().then((doc) => {
          if (doc.exists) {
    ...
    const tasks = db.collection('tasks').doc(this.taskId)
    tasks.get().then((doc) => {
      if (doc.exists) {
